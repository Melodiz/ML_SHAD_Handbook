...
class RubricCityMedianClassifier(ClassifierMixin):
    def __init__(self):
        # Словарь для хранения медиан средних чеков по комбинации modified_rubrics и city
        self.city_median_dict = None

    def fit(self, X, y):
        # Создаем DataFrame на основе X и y для удобства обработки
        data = pd.DataFrame(
            {'modified_rubrics': X['modified_rubrics'], 'city': X['city'], 'average_bill': y})

        # Вычисляем медианы средних чеков для каждой комбинации modified_rubrics и city
        self.city_median_dict = data.groupby(['modified_rubrics', 'city'])[
            'average_bill'].median().to_dict()
        return self
    ...
    def predict(self, X):
        ...
        for idx, row in X.iterrows():
            ...
            if key in self.city_median_dict:
                ...
                predictions.append(
                    np.median(list(self.city_median_dict.values())))
